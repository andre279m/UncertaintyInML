...
positive_pairs_prots = pd.DataFrame(pairs_prots_STRING)
positive_pairs_prots.to_csv('DB/SSMC-master/positive_pairs_prots.tsv', sep='\t', header=False,index=False)
...
for prot in prots_links:
    for prot2 in prots_links:
        if (prot,prot2) in pairs_prots_STRING or (prot2,prot) in pairs_prots_STRING:
            continue
        elif prot != prot2:
