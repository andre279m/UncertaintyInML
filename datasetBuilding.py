...
for i in range(800, -1, -200):
    ...
    for j, (train_index, test_index) in enumerate(skf.split(X, y)):
        ...
        np.savetxt('DB/Static/T'+str(i)+'/_X_train_Fold'+str(j)+'.csv', X_train, delimiter=',')
        np.savetxt('DB/Static/T'+str(i)+'/_X_test_Fold'+str(j)+'.csv', X_test, delimiter=',')
        np.savetxt('DB/Static/T'+str(i)+'/_y_train_Fold'+str(j)+'.csv', y_train, delimiter=',')
        np.savetxt('DB/Static/T'+str(i)+'/_y_test_Fold'+str(j)+'.csv', y_test, delimiter=',')
        np.savetxt('DB/Static/T'+str(i)+'/_sample_weight_train_Fold'+str(j)+'.csv', sample_weight_train, delimiter=',')
...
for i in range(800, -1, -200):
    ...
    for j, (train_index, test_index) in enumerate(skf.split(X, y)):
        ...
        sample_weight_train = sample_weight[train_index]
        logging.info("Writing dataset with threshold: " + str(i) + " and fold: " + str(j))
        np.savetxt('DB/Growing/T'+str(i)+'/X_train_Fold'+str(j)+'.csv', X_train, delimiter=',')
        np.savetxt('DB/Growing/T'+str(i)+'/X_test_Fold'+str(j)+'.csv', X_test, delimiter=',')
...
for i in range(800, -1, -200):
    if i == 800:
        ...
        data0 = data_full.where(data_full['combined_score']<=200).sample(n=setsize,random_state=42)
    ...
    for j, (train_index, test_index) in enumerate(skf.split(X, y)):
        ...
        sample_weight_train = sample_weight[train_index]
        logging.info("Writing dataset with threshold: " + str(i) + " and fold: " + str(j))
        np.savetxt('DB/Uniform/T'+str(i)+'/X_train_Fold'+str(j)+'.csv', X_train, delimiter=',')
        np.savetxt('DB/Uniform/T'+str(i)+'/X_test_Fold'+str(j)+'.csv', X_test, delimiter=',')
