...
for t in ['Uniform','Undersampling']:
    ...
    for m in ['Weighted']:
        for clf in ["RandomForestClassifier","XGBClassifier"]:
            ...
            for i in range(800, -1, -200):
                d = pd.read_csv('../Results/18_05Test/Folds/' + t + 'T' + str(i) + 'metrics_' + m + clf + '.csv',header=0)
            ...
            global_results.to_csv('../Results/18_05Test/' + t + 'metrics_' + m + clf + '.csv')
